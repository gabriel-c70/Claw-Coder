...
> ![claw-coder chat displayed](screenshoot1.png) 
...
- note that versions from claw-coder@0.4.26 to 27 have been deleted from this project and claw-coder version 0.5.0 and above includes an official off button
- Telemetry can be turned off with this command:
- ```bash
  claw-coder telemetry status - to see the current status of telemetry
  claw-coder telemetry on - to turn it on if you want
  claw-coder telemetry off - to turn it off if you want
```
...
claw-coder setup
claw-coder login  - this signs you up so you can get the full capabilities of claw-coder
```
---
...
> TIP
> 
> - `claw-coder setup` installs the Python dependencies from 
> 
> - `requirements.txt`. 
Ollama shall start running for every chat and after claw-coder setup, embeddings, and vector RAG:
---
```bash
ollama serve for old versions of claw-coder but its not needed for new versions for it is initialized automatically after claw-coder.
claw-coder <model>
claw-coder <chat model> <embedding modal>
...
```bash
claw-coder doctor # checks whether everything is all set
claw-coder languages # displays languages supported by claw-coder's tree sitter ability
claw-coder ingest . # ingests the contents of the current directory into the knowledge graph
claw-coder graph "tree_sitter imports" # gets info from the knowledge graph and search for imports of tree-sitter used in a project
claw-coder search "where is graph reranking implemented?" --top-k 5 # this does the same but goes in depth
claw-coder # starts interactive chat (ollama serve is checked/started in the background)
```
- This is a screenshot of claw-coder --help with all the commands displayed

![claw-coder --help displayed](screenshoot3.png)

---
Useful options:

```bash
claw-coder ingest ./src --no-vector-rag
claw-coder search "authentication flow" --graph ./my_graph.json --db ./rag_db
claw-coder graph "calls run_terminal" --top-k 10 --depth 3
```
...
![claw-coder usage displayed](screenshoot2.png)
...
## Caution:
```bash
claw-coder  # in the latest version includes a new workspace feature that is mostly a paid feature
